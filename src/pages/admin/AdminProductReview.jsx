 ...
 *   · Heavy derived lists memoized
 *   · Media inbox cached via workflow service
 *   · Button actions with immediate feedback and busy states
 *   · Paginated inbox rendering (first 24, load more)
 */
...
export default function AdminProductReview() {
  ...
  const queue = useMemo(() => items
    ...
    .sort((a, b) => (a.review.submittedAt < b.review.submittedAt ? 1 : -1)), [items]);

  const drafts = useMemo(() => items
    .filter((product) => product.status === PRODUCT_STATUSES.DRAFT)
    .sort((a, b) => String(a.id).localeCompare(String(b.id))), [items]);

  const kidsDrafts = useMemo(() => drafts.filter((product) => /^KID-\d{3}$/.test(String(product.id))), [drafts]);
  const otherDrafts = useMemo(() => drafts.filter((product) => !/^KID-\d{3}$/.test(String(product.id))), [drafts]);
  ...
  const approve = useCallback((product) => {
    if (busyId) return;
    setBusyId(product.id);
    setNotice({ tone: "ok", text: `Approving ${product.id}…` });
    setTimeout(() => {
  ...
  const reject = useCallback((product) => {
    if (busyId) return;
    setBusyId(product.id);
    setTimeout(() => {
      const result = catalogRepository.rejectProduct(product.id, reason.trim() || "Missing product details.", actor);
  ...
  return (
    ...
    >
      ...
      <div className="mt-8">
        <AdminPanel eyebrow="Kids finalization · 21 confirmed products" title="Kids products">
          <AdminKidsFinalizationPanel actor={actor} onNotice={setNotice} focusId={focusedDraftId && /^KID-\d{3}$/.test(focusedDraftId) ? focusedDraftId : null} />
        </AdminPanel>
      </div>

      {kidsDrafts.length ? (
        <div className="mt-8">
          <AdminPanel eyebrow={`Kids reconciliation · ${kidsDrafts.length} open drafts`} title="Ownership reconciliation">
            <AdminKidsReviewPanel actor={actor} onNotice={setNotice} focusId={focusedDraftId && /^KID-\\d{3}$/.test(focusedDraftId) ? focusedDraftId : null} />
          </AdminPanel>
        </div>
      ) : null}
      ...
      {otherDrafts.length ? (
        <div className="mt-8">
          <AdminPanel eyebrow={`Draft products · ${otherDrafts.length}`} title="Other product drafts">
            <div className="mb-4 flex flex-wrap gap-1.5 border-b border-mist pb-4">
              {otherDrafts.map((draft) => (
                <button
                  ...
                  className={`px-3 py-1.5 font-ui text-[10px] uppercase tracking-[.14em] transition-colors ${focusedDraft?.id === draft.id ? "bg-ink text-ivory" : "text-taupe hover:bg-mist/60 hover:text-ink"}`}
            ...
            {focusedDraft ? (
              <ProductDraftReviewPanel product={focusedDraft} actor={actor} onNotice={setNotice} />
            ) : null}
          </AdminPanel>
        </div>
      ) : null}
      ...
      <div className="mt-8">
        <AdminPanel eyebrow={`Awaiting review · ${queue.length}`} title="Review queue">
          {!queue.length ? (
            <p className="py-10 text-center font-ui text-sm text-taupe">Nothing is waiting for review. The atelier is in order.</p>
          ) : (
                ...
                <thead>
                  <tr className="border-b border-mist font-ui text-[10px] uppercase tracking-widest text-taupe">
                    {["Product", "SKU", "Category", "Price", "Media", "Submitted", "Actions"].map((heading) => (
                      <th key={heading} className="px-3 py-3" scope="col">{heading}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={product.id} className="border-b border-mist/60 align-top font-ui text-sm">
                        <td className="px-3 py-4">
                          <Link to={`/admin/products/${product.id}`} className="underline-offset-4 hover:text-accent hover:underline">{product.name}</Link>
                          <p className="mt-1 text-[11px] text-taupe">{product.review.submittedBy ? `by ${product.review.submittedBy}` : ""}{product.createdBy ? ` · created ${product.createdBy}` : ""}</p>
                          {issues.length ? (<p className="mt-1 text-[11px] text-accent">{issues.length} publishing blocker{issues.length === 1 ? "" : "s"}</p>) : null}
                        </td>
                        <td className="px-3 py-4 text-taupe">{product.sku}</td>
                        <td className="px-3 py-4">{categoryLabels[product.category] ?? product.category}{product.subcategory ? (<span className="block text-[11px] text-taupe">{product.subcategory}</span>) : null}</td>
                        <td className="px-3 py-4">{formatINR(product.price)}</td>
                        <td className="px-3 py-4">{hasCover ? (<StatusBadge label={`✓ Cover · ${summary?.images ?? 0} img · ${summary?.videos ?? 0} vid`} tone="ink" />) : (<StatusBadge label="Needs cover" tone="danger" />)}</td>
                        <td className="px-3 py-4 text-[11px] text-taupe">{product.review.submittedAt ? formatEmployeeDateTime(product.review.submittedAt) : "—"}</td>
                        <td className="px-3 py-4">
                          <div className="flex flex-wrap items-center gap-2">
                            <Link to={`/admin/products/${product.id}`} aria-label={`Open ${product.name}`} title="Open record"><Eye size={15} aria-hidden="true" /></Link>
                            <button type="button" disabled={isBusy} onClick={() => approve(product)} className={`inline-flex items-center gap-1 border border-ink px-3 py-1.5 font-ui text-[10px] uppercase tracking-[.14em] text-ink transition-colors hover:bg-ink hover:text-ivory ${isBusy ? "opacity-40" : ""}`}> <Check size={11} aria-hidden="true" /> {isBusy ? "Approving…" : "Approve"} </button>
                            <button type="button" disabled={isBusy} onClick={() => { setRejectingId(rejectingId === product.id ? null : product.id); setReason(""); }} className={`inline-flex items-center gap-1 border border-mist px-3 py-1.5 font-ui text-[10px] uppercase tracking-[.14em] text-taupe transition-colors hover:border-accent hover:text-accent ${isBusy ? "opacity-40" : ""}`}> <X size={11} aria-hidden="true" /> Reject </button>
                          </div>
                          {rejectingId === product.id ? (
                            <form className="mt-3 space-y-2" onSubmit={(event) => { event.preventDefault(); reject(product); }}>
                              <label htmlFor={`reject-${product.id}`} className="sr-only">Rejection reason for {product.name}</label>
                              <textarea id={`reject-${product.id}`} rows={2} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Missing product details. Incorrect price. Poor product image…" className="w-full border border-mist bg-canvas px-3 py-2 font-ui text-sm outline-none focus:border-accent" />
                              <AtelierButton type="submit" size="chip" disabled={isBusy}>{isBusy ? "Rejecting…" : "Confirm rejection"}</AtelierButton>
                            </form>
                          ) : null}
                        </td>
                      </tr>
            ...
            <ul className="divide-y divide-mist/70">
              {recentlyReviewed.map((product) => (
                <li key={product.id} className="flex flex-wrap items-center justify-between gap-3 px-1 py-3">
                  <div>
                    <Link to={`/admin/products/${product.id}`} className="font-ui text-sm text-ink underline-offset-4 hover:text-accent hover:underline">{product.name}</Link>
                  ...
                  <StatusBadge label={product.review.state === "APPROVED" ? "Approved" : "Rejected"} tone={product.review.state === "APPROVED" ? "ink" : "danger"} />
                </li>
              ))}
            </ul>
          </AdminPanel>
